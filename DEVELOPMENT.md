...
- Supabase Auth με email/κωδικό ή Google και συγχρονισμό πολλών συσκευών, με optimistic revision checks, αυτόματο merge και conflict retry. Ο κύκλος ζωής της ταυτότητας περνά από μηχανή τεσσάρων καταστάσεων (`unknown`, `member`, `offline-member`, `guest`) και ένα μοναδικό σημείο μετάβασης.
- Απομόνωση των τοπικών δεδομένων μεταξύ λογαριασμών και επισκέπτη. Στο επιτυχές sign-out καθαρίζονται τα κοινά domain keys, ενώ διατηρούνται ο owner marker και το user-scoped recovery cache. E2E σενάρια ελέγχουν την εναλλαγή χρήστη και την είσοδο επισκέπτη μετά από αποσύνδεση.
...
- Modular ES αρχιτεκτονική με σαφή όρια για storage/migrations, routines, sessions, progress/rewards, DOM-free παραγωγή του Ιστορικού και κοινά UI primitives. Η κάρτα λογαριασμού, τα dialogs και το interaction-hardening layer βρίσκονται στο ξεχωριστό `dialogs.css`, με μηχανικό έλεγχο της cascade.
...
- **Λειτουργία επισκέπτη** ως απόφαση της συσκευής και όχι λογαριασμός: δύο τοπικά κλειδιά (`logbookGuest`, `logbookGuestReminderAt`), καμία κλήση στο cloud, καμία κλειδωμένη λειτουργία, και μια non-blocking, atomic `aria-live="polite"` λωρίδα υπενθύμισης που επανέρχεται κάθε επτά ημέρες και δεν καλύπτει τα toast.
...
- Κανονικοποίηση των αριθμητικών τιμών και escaping του περιεχομένου χρήστη πριν από απόδοση σε HTML, μαζί με βαθύ validation των local/cloud payloads και επικυρωμένο όριο payload μικρότερο από 2 MiB στη βάση.
- CSP με `default-src 'self'` και `form-action 'self'`. Ο περιορισμός του GitHub Pages για `frame-ancestors` είναι τεκμηριωμένος και απαιτεί μετάβαση σε header CSP αν αλλάξει το hosting.
- Προσβάσιμες κρίσιμες διαδρομές με `inert`/`aria-hidden` στο κρυμμένο auth gate, focus/live-region contracts, axe ελέγχους και τεκμηριωμένο keyboard-only/screen-reader walkthrough.
- Ελεγχόμενο version bump που ενημερώνει μαζί package metadata, UI version, service-worker cache, tests και documentation.
- Release hygiene και ενιαίο quality gate που αποκλείουν debug logging και test artifacts από τον production κώδικα και τα releases. Το production build κάνει minify τα runtime JS/CSS, αντιγράφει μόνο allowlisted αρχεία και ελέγχεται με performance budgets 1.300.000 bytes, LCP 3.500 ms και TBT 500 ms σε emulated mobile συνθήκες.
...
Roadmap με άξονα το εμπορικό μοντέλο: **φάση 1 δωρεάν προϊόν (όχι open source), τελικός στόχος κέρδος από κινητό.** Εδώ καταγράφονται μόνο τα ανοιχτά TODO, με ρητό κριτήριο ολοκλήρωσης όπου χρειάζεται. Οι ολοκληρωμένες δυνατότητες περιγράφονται στην ενότητα «Τι κάνει η εφαρμογή».

### Α. Design & προσβασιμότητα
...
### Β. Εμπορικά — προαπαιτούμενα του 1.0
