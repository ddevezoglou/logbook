...
function renderPlan() {
  ...
  $('#plan-list').innerHTML = planOrder.map((day, dayIndex) => {
    ...
    return `<section class="day-card ${items.length ? 'active-day' : ''}"><div class="day-card-head"><span>${String(dayIndex + 1).padStart(2,'0')}</span><div><h3>${day}</h3><p>${esc(workoutName)}</p></div>${items.length ? `<button class="delete-day" data-delete-day="${day}" aria-label="Διαγραφή ημέρας">×</button>` : ''}</div>
...
$('#plan-form').addEventListener('submit', event => {
  ...
  store.write('trainingPlan', state.plan); event.target.reset(); $('#exercise-count').value = 3; renderPlanExercises(); renderPlan(); renderScheduledSession(); toast(`Η ${day} αποθηκεύτηκε`);
});
...
document.addEventListener('click', event => {
  ...
  if (event.target.dataset.deleteDay) { state.plan = state.plan.filter(x => x.day !== event.target.dataset.deleteDay); store.write('trainingPlan', state.plan); renderPlan(); renderScheduledSession(); }
...
$('#log-date').valueAsDate = new Date(); renderPlanExercises(); renderPlan(); renderScheduledSession(); renderOverview();
